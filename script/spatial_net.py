...
def VGG_gp(x_sat, x_grd, keep_prob, trainable):
    ...
    grd_global = tf.nn.max_pool(
        grd_vgg, [1, grd_height, grd_width, 1], [1, 1, 1, 1], padding="VALID"
    )
    grd_global = tf.reshape(grd_global, [-1, grd_channel])

    sat_height, sat_width, sat_channel = sat_vgg.get_shape().as_list()[1:]
    sat_global = tf.nn.max_pool(
        sat_vgg, [1, sat_height, sat_width, 1], [1, 1, 1, 1], padding="VALID"
    )
