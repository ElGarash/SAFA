...
class InputData:
    ...
    def next_batch_scan(self, batch_size):
        ...
        batch_grd = np.zeros(
            [batch_size, self.panoRows, self.panoCols, 3], dtype=np.float32
        )
        batch_sat = np.zeros(
            [batch_size, self.satSize, self.satSize, 3], dtype=np.float32
        )
        batch_grd_yawpitch = np.zeros(
            [batch_size, self.panoRows, self.panoCols, 2], dtype=np.float32
        )
        batch_sat_yaw = np.zeros(
            [batch_size, self.satSize, self.satSize, 2], dtype=np.float32
        )
        ...
        for i in range(batch_size):
            ...
            img = cv2.resize(
                img, (self.panoCols, self.panoRows), interpolation=cv2.INTER_AREA
            )
            img = img.astype(np.float32)
            # normalize it to -1 --- 1
            img[:, :, 0] -= 103.939  # Blue
            ...
            batch_sat_yaw[i, :, :, 0] = cv2.resize(
                self.yaw_sat["polor_mat"][:, :, 0].astype(np.float32) / np.pi,
                (self.satSize, self.satSize),
                interpolation=cv2.INTER_AREA,
            )

            batch_sat_yaw[i, :, :, 1] = cv2.resize(
                (self.yaw_sat["polor_mat"][:, :, 1].astype(np.float32) - 0.5) * 2.0,
                (self.satSize, self.satSize),
                interpolation=cv2.INTER_AREA,
            )
    ...
    def next_pair_batch(self, batch_size):
        ...
        while True:
            ...
            img_radis = (
                self.yaw_sat["polor_mat"][:, :, 1].astype(np.float32) - 0.5
            ) * 2.0
            ...
            if rand_crop > self.satSize:
                ...
                img_radis = img_radis[
                    start : start + rand_crop, start : start + rand_crop
                ]
            ...
            img_yaw = cv2.resize(
                img_yaw, (self.satSize, self.satSize), interpolation=cv2.INTER_AREA
            )
            img_radis = cv2.resize(
                img_radis, (self.satSize, self.satSize), interpolation=cv2.INTER_AREA
            )

            rand_rotate = random.randint(0, 4) * 90
            rot_matrix = cv2.getRotationMatrix2D(
                (self.satSize / 2, self.satSize / 2), rand_rotate, 1
            )
            ...
            img_radis = cv2.warpAffine(
                img_radis, rot_matrix, (self.satSize, self.satSize)
            )
